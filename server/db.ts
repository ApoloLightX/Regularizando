...
export async function qualifyPilotRequest(input: {
  ...
  if (previous.requestCategory !== "pilot") throw new Error("Pedidos de privacidade não podem ser qualificados como lead comercial.");
...
export async function getDataGovernanceOverview(organizationId: number) { const db = await getDb(); if (!db) throw new Error("Banco de dados indisponível"); const [policies, requests, policyVersions, requestEvents, team] = await Promise.all([db.select().from(dataRetentionPolicies).where(eq(dataRetentionPolicies.organizationId, organizationId)), db.select().from(dataSubjectRequests).where(eq(dataSubjectRequests.organizationId, organizationId)).orderBy(desc(dataSubjectRequests.createdAt)), db.select().from(dataRetentionPolicyVersions).where(eq(dataRetentionPolicyVersions.organizationId, organizationId)).orderBy(desc(dataRetentionPolicyVersions.recordedAt)), db.select().from(dataSubjectRequestEvents).where(eq(dataSubjectRequestEvents.organizationId, organizationId)).orderBy(desc(dataSubjectRequestEvents.createdAt)), getTeamOverview(organizationId)]); return { policies, requests, policyVersions, requestEvents, members: team.members }; }
...
export async function assignDataSubjectRequest(input: { requestId: number; organizationId: number; assignedToUserId: number; assignedByUserId: number }) { const db = await getDb(); if (!db) throw new Error("Banco de dados indisponível"); const member = await getMemberOfOrganization(input.organizationId, input.assignedToUserId); if (!member) throw new Error("O responsável precisa ser membro da organização."); const result = await db.update(dataSubjectRequests).set({ assignedToUserId: input.assignedToUserId, assignedAt: new Date() }).where(and(eq(dataSubjectRequests.id, input.requestId), eq(dataSubjectRequests.organizationId, input.organizationId))); if (result[0].affectedRows !== 1) throw new Error("Pedido LGPD não encontrado na organização atual."); return (await db.select().from(dataSubjectRequests).where(eq(dataSubjectRequests.id, input.requestId)).limit(1))[0]; }
...
export async function handleDataSubjectRequest(input: { requestId: number; organizationId: number; handledByUserId: number; status: "nova" | "em_analise" | "aguardando_informacoes" | "aprovada" | "rejeitada" | "executada" | "encerrada"; decisionRationale: string; executionNote?: string | null }) { const db = await getDb(); if (!db) throw new Error("Banco de dados indisponível"); const now = new Date(); const result = await db.update(dataSubjectRequests).set({ status: input.status, decisionRationale: input.decisionRationale, handledByUserId: input.handledByUserId, handledAt: now, executionNote: input.executionNote ?? null, executedAt: input.status === "executada" ? now : null, closedAt: input.status === "encerrada" ? now : null }).where(and(eq(dataSubjectRequests.id, input.requestId), eq(dataSubjectRequests.organizationId, input.organizationId))); if (result[0].affectedRows !== 1) throw new Error("Pedido LGPD não encontrado na organização atual."); return (await db.select().from(dataSubjectRequests).where(eq(dataSubjectRequests.id, input.requestId)).limit(1))[0]; }
