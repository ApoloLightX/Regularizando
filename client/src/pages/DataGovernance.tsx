...
const requestStatuses = ["nova", "em_analise", "aguardando_informacoes", "aprovada", "rejeitada", "executada", "encerrada"] as const;
...
const labels: Record<string, string> = {
  ...
  rascunho: "Rascunho", em_revisao: "Em revisão", ativa: "Ativa", nova: "Nova", em_analise: "Em análise", aguardando_informacoes: "Aguardando informações", aprovada: "Aprovada", rejeitada: "Rejeitada", executada: "Executada", encerrada: "Encerrada",
  revisao_manual: "Revisão manual", anonimizacao_revisada: "Anonimização revisada", exclusao_revisada: "Exclusão revisada",
};

function statusClass(status: string) {
  if (["ativa", "aprovada", "executada", "encerrada"].includes(status)) return "status-chip status-chip--verified";
  if (["rejeitada"].includes(status)) return "status-chip status-chip--vencida";
...
export default function DataGovernance() {
  ...
  const assignRequest = trpc.dataGovernance.assignSubjectRequest.useMutation({ onSuccess: () => void utils.dataGovernance.overview.invalidate() });
  ...
  const [executionNotes, setExecutionNotes] = useState<Record<number, string>>({});
  const [requestStatusesById, setRequestStatusesById] = useState<Record<number, (typeof requestStatuses)[number]>>({});
  const [assigneesById, setAssigneesById] = useState<Record<number, string>>({});
  ...
  async function decideRequest(requestId: number) {
    const status = requestStatusesById[requestId] ?? "nova";
    await handleRequest.mutateAsync({ requestId, status, decisionRationale: decisionNotes[requestId] ?? "", executionNote: executionNotes[requestId]?.trim() || undefined });
  }

  async function assignResponsible(requestId: number) {
    const assignedToUserId = Number(assigneesById[requestId]);
    if (!Number.isInteger(assignedToUserId) || assignedToUserId <= 0) return;
    await assignRequest.mutateAsync({ requestId, assignedToUserId });
  }
  ...
  return <DashboardLayout><div className="dashboard-page evidence-page">
    ...
    <section className="dashboard-card mt-6" aria-labelledby="lgpd-baseline"><div className="card-heading"><div><span className="kicker">Dois níveis de governança</span><h2 id="lgpd-baseline">Baseline da plataforma e política da organização</h2></div><Scale size={21} aria-hidden="true" /></div><div className="grid gap-4 md:grid-cols-2 mt-4"><p><strong>Baseline do Regularizando.</strong><br />Minimização, controle de acesso, registros versionados, revisão humana e ausência de descarte automático. Não substitui decisão jurídica ou contratual.</p><p><strong>Política da organização.</strong><br />Prazo, justificativa, responsável e método de descarte são configurados por categoria e ficam versionados. A ativação não executa exclusão por si só.</p></div></section>
    ...
    {overview.isLoading ? <section className="dashboard-card"><p>Carregando registros de governança…</p></section> : <div className="grid gap-6 xl:grid-cols-2 mt-6">
      <section className="dashboard-card"><div className="card-heading"><div><span className="kicker">Retenção</span><h2>Política por categoria</h2></div><Scale size={21} aria-hidden="true" /></div><p className="dashboard-subtitle">Ativar uma política formaliza a aprovação atual; a execução de descarte continua dependente de revisão humana.</p>
        ...
        <label>Base legal e justificativa<textarea value={legalBasisNote} onChange={(event) => setLegalBasisNote(event.target.value)} minLength={12} placeholder="Descreva finalidade, fundamento proposto e justificativa. Exige revisão jurídica antes de clientes." /></label>
    ...
    <section className="dashboard-card mt-6"><div className="card-heading"><div><span className="kicker">Atendimento</span><h2>Fila de pedidos LGPD</h2></div><ClipboardList size={21} aria-hidden="true" /></div>{data?.requests.length ? <div className="compact-list mt-4">{data.requests.map((request) => { const events = data.requestEvents.filter((event) => event.requestId === request.id); const member = data.members.find((item) => item.user.id === request.assignedToUserId); const selectedStatus = requestStatusesById[request.id] ?? request.status; return <div className="compact-list__row block md:flex" key={request.id}><div className="min-w-0"><strong>{labels[request.requestType]} <span className="text-muted-foreground font-normal">· referência pseudonimizada</span></strong><small>Recebido em {new Date(request.createdAt).toLocaleDateString("pt-BR")} · {request.scopeNote}</small><small>Responsável: {member?.user.name || member?.user.email || "Não atribuído"}</small>{request.decisionRationale ? <small>Decisão: {request.decisionRationale}</small> : null}{request.executionNote ? <small>Execução: {request.executionNote}</small> : null}{events.map((event) => <small key={event.id}>{event.eventType}: {event.evidenceReference || "registro interno"} · {event.note}</small>)}</div><div className="mt-3 md:mt-0 md:ml-4 md:w-80"><label>Responsável<select aria-label={`Responsável do pedido ${request.id}`} value={assigneesById[request.id] ?? request.assignedToUserId ?? ""} onChange={(event) => setAssigneesById((current) => ({ ...current, [request.id]: event.target.value }))}><option value="">Selecionar membro</option>{data.members.map((item) => <option key={item.user.id} value={item.user.id}>{item.user.name || item.user.email || `Usuário ${item.user.id}`}</option>)}</select></label><button type="button" className="button button--outline mt-2" disabled={assignRequest.isPending || !assigneesById[request.id]} onClick={() => void assignResponsible(request.id)}>{assignRequest.isPending ? "Atribuindo…" : "Atribuir responsável"}</button><div className="flex gap-2 items-center mt-3"><select aria-label={`Estado do pedido ${request.id}`} value={selectedStatus} onChange={(event) => setRequestStatusesById((current) => ({ ...current, [request.id]: event.target.value as (typeof requestStatuses)[number] }))}>{requestStatuses.map((item) => <option key={item} value={item}>{labels[item]}</option>)}</select><span className={statusClass(request.status)}>{labels[request.status]}</span></div><textarea aria-label={`Justificativa da decisão ${request.id}`} value={decisionNotes[request.id] ?? ""} onChange={(event) => setDecisionNotes((current) => ({ ...current, [request.id]: event.target.value }))} placeholder="Justificativa da análise ou decisão" />{selectedStatus === "executada" ? <textarea aria-label={`Registro de execução ${request.id}`} value={executionNotes[request.id] ?? ""} onChange={(event) => setExecutionNotes((current) => ({ ...current, [request.id]: event.target.value }))} placeholder="Ação executada e resultado" /> : null}<button type="button" className="button button--outline mt-2" disabled={handleRequest.isPending || (decisionNotes[request.id] ?? "").trim().length < 12 || (selectedStatus === "executada" && (executionNotes[request.id] ?? "").trim().length < 12)} onClick={() => void decideRequest(request.id)}>{handleRequest.isPending ? "Salvando…" : "Registrar etapa"}</button><input aria-label={`Referência de evidência ${request.id}`} value={evidenceReferences[request.id] ?? ""} onChange={(event) => setEvidenceReferences((current) => ({ ...current, [request.id]: event.target.value }))} placeholder="Referência de evidência (opcional)" /><textarea aria-label={`Nota de evidência ${request.id}`} value={eventNotes[request.id] ?? ""} onChange={(event) => setEventNotes((current) => ({ ...current, [request.id]: event.target.value }))} placeholder="Nota, evidência ou marco de atendimento" /><button type="button" className="button button--outline mt-2" disabled={recordRequestEvent.isPending || (eventNotes[request.id] ?? "").trim().length < 12} onClick={() => void recordEvidence(request.id)}>{recordRequestEvent.isPending ? "Registrando…" : "Registrar evidência"}</button></div></div>; })}</div> : <div className="empty-state"><span><ShieldAlert size={21} /></span><div><strong>Nenhum pedido registrado</strong><p>Registre pedidos de titulares quando houver base operacional e contexto suficiente para revisão humana.</p></div></div>}</section>
  </div></DashboardLayout>;
}
